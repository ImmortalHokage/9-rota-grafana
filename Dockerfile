...
# Update Title
RUN sed -i 's|<title>\[\[.AppTitle\]\]</title>|<title>9 РОТА logs</title>|g' /usr/share/grafana/public/views/index.html
...
## Update Title
RUN find /usr/share/grafana/public/build/ -name *.js -exec sed -i 's|AppTitle="Grafana"|AppTitle="9 РОТА logs"|g' {} \;

## Update Login Title
RUN find /usr/share/grafana/public/build/ -name *.js -exec sed -i 's|LoginTitle="Welcome to Grafana"|LoginTitle="Добро пожаловать в систему логирования"|g' {} \;
